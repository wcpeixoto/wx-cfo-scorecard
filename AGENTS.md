...
4. `AGENTS.md` — execution rules and workflow discipline
...
3. `UI_CARDS.md` — card anatomy, height behavior, pairing classification
4. This file (`AGENTS.md`) — project rules, stack, and working discipline
...
1. Read `UI_RULES.md` before any UI code. For any new UI pattern,
   ...
   This is in addition to the standard requirement to read `AGENTS.md` and `UI_RULES.md`. `UI_CARDS.md` is the source of truth for dashboard card anatomy, fixed vs optional elements, TailAdmin card scale, spacing rhythm, and the CashTrendHero implementation contract.
...
must re-upload that file to the Codex project before closing.

Documentation files that must stay in sync with the Codex project:
- `UI_RULES.md`
- `UI_CARDS.md`
- `AGENTS.md`
- `wx_cfo_scorecard_context_v2_6.md`
...
□ Re-upload changed .md files to the Codex project (Codex.ai → project → files)
□ Confirm Codex project files match repo state
□ Do not close the session until the upload is confirmed

Why this exists: Codex project files are snapshots, not live repo files.
Every commit that changes a doc file makes the Codex project stale.
Stale project files cause Codex to read outdated specs and produce wrong guidance.
